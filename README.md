...
如需查看每一步的结构化中间数据，可以开启 debug 模式：

```bash
python main.py --debug "北京夏季男性短袖购买，材质工艺挑选"
```

debug 输出会显示初始搜索结果、问题分析与搜索计划、每轮搜索结果、网页抓取记录、未抓取正文的搜索摘要候选、检索评估、路由决策、证据精炼、来源摘要和报告校验结果。长正文会自动截断，避免终端输出过大。
